...
def g():
    ...
    model = mr.get_best_model("twitter_bitcoin_sentiment_2", 'f1', 'max')
    model_dir = model.download()
    model = joblib.load(model_dir + "/twitter_bitcoin_sentiment_model_2.pkl")
    ...
    img.save("./latest_bitcoin_fluctuation_prediction_2.png")
    dataset_api = project.get_dataset_api()    
    dataset_api.upload("./latest_bitcoin_fluctuation_prediction_2.png", "Resources/images", overwrite=True)
    ...
    img.save("./latest_bitcoin_fluctuation_actual_2.png")
    dataset_api.upload("./latest_bitcoin_fluctuation_actual_2.png", "Resources/images", overwrite=True)
    
    # get prediction feature group from hopsworks or create new one
    monitor_fg = fs.get_or_create_feature_group(name="twitter_bitcoin_sentiment_predictions_2",
                                                version=1,
                                                primary_key=["datetime"],
                                                description="Twitter bitcoin sentiment 2 Prediction/Outcome Monitoring"
                                                )
    ...
    model_dir="twitter_bitcoin_sentiment_predictions_2"
    if os.path.isdir(model_dir) == False:
        os.mkdir(model_dir)
    df_recent = history_df.tail(5)
    dfi.export(df_recent, 'twitter_bitcoin_sentiment_predictions_2/df_recent_2.png', table_conversion = 'matplotlib')
    dataset_api.upload("twitter_bitcoin_sentiment_predictions_2/df_recent_2.png", "Resources/images", overwrite=True)
    ...
    if predictions.value_counts().count() == 3:
        ...
        fig = cm.get_figure()
        fig.savefig("twitter_bitcoin_sentiment_predictions_2/confusion_matrix_2.png")
        # save confusion matrix image to dataset api
        dataset_api.upload("twitter_bitcoin_sentiment_predictions_2/confusion_matrix_2.png", "Resources/images", overwrite=True)
    else:
...
if __name__ == "__main__":
    if LOCAL == True :
        ...
        stub.deploy("bitcoin_sentiment_batch_inference_pipeline_daily_2")
